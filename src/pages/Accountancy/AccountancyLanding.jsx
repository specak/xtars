...
const SidebarModuleButton = ({ id, title, Icon, active, onClick, completed }) => (
  <button
    onClick={() => onClick(id)}
...
export default function AccountancyLanding() {
  ...
  const [completedModules, setCompletedModules] = useState(["assets", "liabilities", "equity", "revenue"]);
  const { theme, setTheme } = useTheme();  // Initialize theme toggle

  useEffect(() => {
    // Optional: Automatically adjust the theme based on user's preference or default setting.
    setTheme('dark');
  }, []);
